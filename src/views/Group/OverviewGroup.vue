                ...
                <div class="center-group">
                    <div class="group">
                        ...
                        <div class="tab-group-title">
                            ...
                            <div class="flex">
                                <InputIcon class="mg-r-10" icon="icon-search" placeholder="Tìm kiếm nhóm" :autoFocus="true"/>
                                <ButtonText :Width="120" text="Tạo nhóm mới" @click.native="showCreateGroup"/>
                            </div>
                        </div>
                        <!-- List render nhóm -->
                        <div class="tab-group-render">
                            <div class="item-group" v-for="(item, index) of listGroup" :key="index">
                                <div class="image-group">
                                    <!-- <img :src="item.background.imageURL" alt=""> -->
                                    <cld-image 
                                        :publicId="item.background.cloudinaryID ? item.background.cloudinaryID : imageDefault" 
                                        loading="lazy">
                                        <cld-transformation width="100" gravity="south" crop="fill"/>
                                    </cld-image>
                                </div>
                                <div class="title-group">
                                    <div class="name-group" @click="accessGroup(item)">{{ item.name }}</div>
                                    <div class="general">{{ item.general }}</div>
                                </div>
                                <div class="icon-34 icon-wrap">
                                    <div class="icon-20 icon-joined"></div>
                                </div>
                            </div>
                        </div>
                        <div class="tab-load-more" @click="loadMoreGroup" v-if="pageIndex < totalPage">{{ $t('i18nGroup.OverviewGroup.LoadMore') }}</div>
                    </div>
                </div>
            </template>
            <template v-slot:right-bar>
                <div class="rightbar-group">
                    <div class="sidebar-right">
                        <div class="title-rightbar">{{ $t('i18nGroup.OverviewGroup.TitleSuggest') }}</div>
                        <div class="suggest">{{ $t('i18nGroup.OverviewGroup.ContentSuggest') }}</div>
                    </div>
                </div>
            </template>
        </Overview>
...
.icon-joined{
    background-image: url(https://static.xx.fbcdn.net/rsrc.php/v3/y_/r/xYnteFyX4Gg.png);
